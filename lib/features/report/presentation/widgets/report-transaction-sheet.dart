...
class ReportTransactionSheet extends ConsumerStatefulWidget {
  ...
  const ReportTransactionSheet({super.key, required this.transaction});

  @override
  ConsumerState<ConsumerStatefulWidget> createState() =>
      _ReportTransactionSheetState();
...
class _ReportTransactionSheetState
    extends ConsumerState<ReportTransactionSheet> {
    ...
    return Column(
      children: [
        ...
        InputFormField(
          ...
          label: 'Can you tell us more about what was wrong?',
          lines: 5,
          validator: (value) {
            return null;
          },
    ...
    return Column(
      children: [
        ...
        DropdownInputFormField(
          ...
          hint: "Reason for reporting this transaction?",
          onValidate: (value) {
            if (value == null) {
    ...
    return CustomButton(
      ...
      onPressed: () async {
        ...
        if (formKey.currentState!.validate()) {
          ...
          final response = await TransactionRepo.reportTransaction(
              transactionId: transaction.transactionId,
              reason: "$selectedReason: ${description ?? ""}".trim());
          if (response.error) {
            SnackbarManager.showErrorSnackbar(
                context: context,
                message: "Error occurred when reporting transaction");
            context.pop(result: false);
          }
          SnackbarManager.showBasicSnackbar(
              context: context, message: "Reported transaction successfully");
